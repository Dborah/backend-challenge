...
class ScheduleAPI(Resource):
    ...
    def post(self):
        ...
        schedule.save()
        resp = [{'message': 'New scheduling created successfully'}]
        return resp, 201
...
class ScheduleItemAPI(Resource):
...
class ScheduleListAPI(Resource):
    ...
    def get(self):
        date = self.args['date']
        query_schedules = Scheduling.filter_date_scheduling(date)
        response = schedule_serializer(query_schedules)
        return response


def init_app(app):
    api.add_resource(ScheduleAPI, '/schedules')
    api.add_resource(ScheduleItemAPI, '/schedules/<int:scheduling_id>')
    api.add_resource(ScheduleListAPI, '/schedules/schedule')
